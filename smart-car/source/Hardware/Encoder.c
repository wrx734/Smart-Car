...
void Encoder_Init(void)
{
	...
	// 2. ⚠️ 必须释放PB4：关闭JTAG，仅保留SWD调试  PB4默认是JNTRST调试引脚，不关闭无法做普通GPIO/复用功能
	...
	GPIO_PinRemapConfig(GPIO_PartialRemap_TIM3, ENABLE);  // TIM3 部分重映射：CH1→PB4 CH2→PB5
	...
	TIM_TimeBaseInit(TIM4, &TIM_TimeBaseInitStructure1);             //将结构体变量交给TIM_TimeBaseInit，配置TIM1的时基单元
	...
	TIM_ICInitTypeDef TIM_ICInitStructure4;							//定义结构体变量
	TIM_ICStructInit(&TIM_ICInitStructure4);							//结构体初始化，若结构体没有完整赋值
																	//则最好执行此函数，给结构体所有成员都赋一个默认值
																	//避免结构体初值不确定的问题
	TIM_ICInitStructure4.TIM_Channel = TIM_Channel_1;				//选择配置定时器通道1
	TIM_ICInitStructure4.TIM_ICFilter = 0xF;							//输入滤波器参数，可以过滤信号抖动
	TIM_ICInit(TIM3, &TIM_ICInitStructure4);							//将结构体变量交给TIM_ICInit，配置TIM3的输入捕获通道
	TIM_ICInitStructure4.TIM_Channel = TIM_Channel_2;				//选择配置定时器通道2
	TIM_ICInitStructure4.TIM_ICFilter = 0xF;							//输入滤波器参数，可以过滤信号抖动
	TIM_ICInit(TIM4, &TIM_ICInitStructure4);							//将结构体变量交给TIM_ICInit，配置TIM3的输入捕获通道
...
//	speed = TIM_GetCounter(TIM4);
	Temp = 199*TIM_GetCounter(TIM4)/133;		//编码器计数换算为速度
